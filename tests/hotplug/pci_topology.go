...
var _ = Describe("[sig-compute]PCI Topology", decorators.SigCompute, func() {
	...
	Context("virt-handler topology detection", func() {
		It("[test_id:US75]should re-detect v3 topology when annotation is removed from running VMI", func() {
			...
			patchPayload, err := patch.New(
				patch.WithRemove("/metadata/annotations/" + patch.EscapeJSONPointer(v1.PciTopologyVersionAnnotation)),
			).GeneratePayload()
			Expect(err).ToNot(HaveOccurred())
			_, err = virtClient.VirtualMachineInstance(vm.Namespace).Patch(context.Background(), vm.Name, types.JSONPatchType, patchPayload, metav1.PatchOptions{})
			Expect(err).ToNot(HaveOccurred())

			By("Waiting for virt-handler to re-detect and re-apply v3 annotation")
			Eventually(func(g Gomega) {
				vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(vmi.Annotations).To(HaveKeyWithValue(v1.PciTopologyVersionAnnotation, v1.PciTopologyVersionV3))
		...
		It("[test_id:US81]should re-detect v2 topology when annotations are removed from running VMI", func() {
			...
			By("Patching VM template with v2 annotations and frozen slot count of 9")
			patchPayload, err := patch.New(
				patch.WithReplace("/spec/template/metadata/annotations/"+patch.EscapeJSONPointer(v1.PciTopologyVersionAnnotation), v1.PciTopologyVersionV2),
				patch.WithAdd("/spec/template/metadata/annotations/"+patch.EscapeJSONPointer(v1.PciInterfaceSlotCountAnnotation), "9"),
			).GeneratePayload()
			...
			By("Removing PCI topology annotations from the running VMI")
			patchPayload, err = patch.New(
				patch.WithRemove("/metadata/annotations/"+patch.EscapeJSONPointer(v1.PciTopologyVersionAnnotation)),
				patch.WithRemove("/metadata/annotations/"+patch.EscapeJSONPointer(v1.PciInterfaceSlotCountAnnotation)),
			).GeneratePayload()
			Expect(err).ToNot(HaveOccurred())
			_, err = virtClient.VirtualMachineInstance(vm.Namespace).Patch(context.Background(), vm.Name, types.JSONPatchType, patchPayload, metav1.PatchOptions{})
			Expect(err).ToNot(HaveOccurred())

			By("Waiting for virt-handler to re-detect and re-apply v2 annotations")
			Eventually(func(g Gomega) {
				vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(vmi.Annotations).To(HaveKeyWithValue(v1.PciTopologyVersionAnnotation, v1.PciTopologyVersionV2))
				g.Expect(vmi.Annotations).To(HaveKeyWithValue(v1.PciInterfaceSlotCountAnnotation, "9"))
			}, 60*time.Second, 2*time.Second).Should(Succeed())
		})
	})
