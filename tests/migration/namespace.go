...
var _ = Describe(SIG("Live Migration across namespaces", decorators.RequiresDecentralizedLiveMigration, func() {
	...
	deleteMigration := func(migration *virtv1.VirtualMachineInstanceMigration) {
		...
		Expect(libwait.WaitForMigrationToDisappearWithTimeout(migration, time.Second*30)).To(Succeed())
	}
	...
	Context("container disk", func() {
		...
		Context("with RWOFs backend storage class", func() {
			...
			// for shared storage.
			It("[test_id:US70]should decentralized migrate a VMI with persistent TPM+EFI enabled", decorators.RequiresDecentralizedLiveMigration, decorators.RequiresRWOFsVMStateStorageClass, Serial, func() {
				migrationID := fmt.Sprintf("mig-%s", rand.String(5))
