			...
			DescribeTable("should migrate the source volume from a source DV to a destination PVC", func(mode string) {
				...
				Eventually(func() error {
					err := console.SafeExpectBatch(vmi, []expect.Batcher{
						&expect.BSnd{S: "\n"},
						&expect.BExp{R: ""},
						&expect.BSnd{S: "[ $(cat /sys/class/block/vda/size) -eq " + expectedSize + " ] && echo 0 || echo 1\n"},
						&expect.BExp{R: "0"},
					}, 10)
					return err
				}, 120).Should(Succeed())
	...
	Context("Hotplug volumes", func() {
		...
		Context("should be able to volume migrate a VM", func() {
			...
			createFileOnHotpluggedVol := func(vmi *v1.VirtualMachineInstance, volName string) {
				...
				Expect(console.RunCommand(vmi, "mkdir /home/alpine/test", 30*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, fmt.Sprintf("sudo mount -t ext3 /dev/%s /home/alpine/test", device), 30*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, "sudo chmod 777 /home/alpine/test", 30*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, "sudo chown alpine:alpine /home/alpine/test", 30*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, "printf 'test' &> /home/alpine/test/test", 30*time.Second)).To(Succeed())
			}
			checkFileOnHotpluggedVol := func(vmi *v1.VirtualMachineInstance) {
				Expect(console.LoginToAlpine(vmi)).To(Succeed())
				Expect(console.RunCommand(vmi, "cat /home/alpine/test/test |grep test", 60*time.Second)).To(Succeed())
			}
